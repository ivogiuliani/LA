...
Regole:
  ...
  - quando l'utente mostra interesse: chiede nome/email/telefono e propone la
    call (booking_url o due finestre, mattine LA) → tool `capture_lead` →
...
CLI:
    ...
    python3 chat_api.py --self-test          # backend finto, nessuna API: verifica il flusso completo
    python3 chat_api.py --self-test --live   # come sopra ma con Claude reale (usa ANTHROPIC_API_KEY)
"""
...
from lead_settings import get as cfg_get, private_dir, SYSTEM_DIR, PROJECT_ROOT
...
DRY_RUN = False
BACKEND = "anthropic"       # "anthropic" | "fake"
_sessions: dict = {}
...
CAPTURE_TOOL = {
    "name": "capture_lead",
    "description": ("Record the visitor's contact details so the founder's office can follow up. "
                    "Call it ONLY after the visitor has given at least a first name and an email "
                    "address in this conversation. Never invent or guess any field."),
    "input_schema": {
        "type": "object",
        "properties": {
...
{sched}. The moment the visitor gives a first name and an email address, call the capture_lead
tool IN THAT SAME TURN with what they told you (do not wait for the phone or the time windows:
ask for those afterwards, in the confirmation), then confirm that the founder's office will reply
{cfg_get('canonical.response_promise', 'within one business day')}. Call the tool at most once
per conversation. Never call it with guessed data. Never ask for financial details, IDs or passwords.

Keep answers under 120 words unless the visitor asks for detail. Do not use markdown headers.
...
def _anthropic_turn(messages: list, session: dict, sid: str) -> str:
    import anthropic
    from model_resolver import resolve
    client = anthropic.Anthropic(api_key=_load_key())
    model = resolve("cheap")
    sysm = system_prompt()
    resp = client.messages.create(model=model, max_tokens=600, system=sysm,
                                  messages=messages, tools=[CAPTURE_TOOL])
    text_parts = [b.text for b in resp.content if getattr(b, "type", "") == "text"]
    tool_uses = [b for b in resp.content if getattr(b, "type", "") == "tool_use"]
    if not tool_uses:
        return "".join(text_parts).strip()
    # esegui il tool e chiudi il giro con un secondo messaggio
    assistant_blocks = []
    ...
    results = []
    for tu in tool_uses:
        out = capture_lead(dict(tu.input or {}), session, sid) if tu.name == "capture_lead" else {"ok": False, "error": "unknown tool"}
        results.append({"type": "tool_result", "tool_use_id": tu.id, "content": json.dumps(out)})
    follow = client.messages.create(
        model=model, max_tokens=400, system=sysm, tools=[CAPTURE_TOOL],
        messages=messages + [{"role": "assistant", "content": assistant_blocks},
                             {"role": "user", "content": results}])
    final = "".join(b.text for b in follow.content if getattr(b, "type", "") == "text").strip()
    return ("".join(text_parts).strip() + "\n\n" + final).strip() if text_parts else final
...
def llm_turn(messages: list, session: dict, sid: str) -> str:
    ...
    try:
        return _anthropic_turn(messages, session, sid)
    except Exception as exc:  # noqa: BLE001
...
def self_test(live: bool) -> int:
    ...
    BACKEND = "anthropic" if live else "fake"
    DRY_RUN = True
