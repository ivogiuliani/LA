...
- Social (2026-09-22): icone Instagram + LinkedIn nell'header dopo Contact (o prima del link Back; marker MV-NAVSOCIAL)
  e nel footer subito dopo il blocco Contact (marker MV-SOCIAL).
...
NSOC_S, NSOC_E = "<!-- MV-NAVSOCIAL:START -->", "<!-- MV-NAVSOCIAL:END -->"
...
def _nav_social_html(solo: bool) -> str:
    return (f'<span class="nav-social{" nav-social--solo" if solo else ""}" aria-label="Social profiles">'
...
CSS = ('<style id="mv-contact-css">'
       ...
       '.nav-social{display:inline-flex;align-items:center;gap:10px;margin:0 6px 0 2px;vertical-align:middle}'
       '.nav-social a{display:inline-flex;color:var(--warm-sand,#C4A265);opacity:.85;transition:opacity .3s}'
       '.nav-social a:hover{opacity:1}'
       '.nav-social svg{width:15px;height:15px;fill:currentColor;display:block}'
       '.nav-social--solo{margin-left:auto;margin-right:18px}'
       '.nav-contact--solo+.nav-social{margin-left:0}'
...
def patch(path: Path, dry: bool) -> dict:
    ...
    # ── social nell'header: dopo il link Contact del menu; senza .nav-cta, prima del link "Back" ──
    rep["nav_social"] = "skip"
    has_navcontact = NAV_E in html
    nblock = _wrap(NSOC_S, NSOC_E, _nav_social_html(solo=not has_navcontact))
    html, changed = _replace_block(html, NSOC_S, NSOC_E, nblock)
    if changed is None:
        if has_navcontact:
            k = html.find(NAV_E) + len(NAV_E)
            html = html[:k] + nblock + html[k:]
            rep["nav_social"] = "inserted"
        else:
            npos = html.find("<nav")
            m = re.search(r'[ \t]*<a [^>]*class="nav-back"[^>]*>', html[npos:]) if npos != -1 else None
            if m:
                start = npos + m.start()
                indent = re.match(r"[ \t]*", m.group(0)).group(0)
                html = html[:start] + f"{indent}{nblock}\n" + html[start:]
                rep["nav_social"] = "inserted"
    else:
        rep["nav_social"] = "updated" if changed else "unchanged"
    ...
    # ── css ──
    if rep["nav"] != "skip" or rep["social"] != "skip" or rep["nav_social"] != "skip":
